...
#-----------------------------------------------------------------------
	if s['CreateDistn']:
		Dx = 0.
		Dxp = 0.
		if p['Optics'] is 'Lattice':
			Dx  = Lattice.etax0
			Dxp = Lattice.etapx0
		elif p['Optics'] is 'Operational':
			Dx  = 2.633
			Dxp = -0.1104
		elif p['Optics'] is 'Rematched':
			Dx  = 2.6829
			Dxp = -0.0221
		...
		Particle_distribution_file = generate_initial_distribution_synch_particle_manual_Twiss(p, twiss_dict, output_file='input/ParticleDistribution.in', summary_file='input/ParticleDistribution_summary.txt')

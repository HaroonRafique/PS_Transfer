...
#-----------------------------------------------------------------------    
    ...
    for i, file in enumerate(files):
        ...
        if not file_exists:
            ...
            plt.plot(x_, gaussian_(x_,popt[0],popt[1], 0, popt[3]), color = 'b', lw = 2,label = str(r'$\sigma = $' + str(round(popt[3],3)) + ' mm'))  
            plt.hist((a[1][:-1] + x_offset), len(a[1][:-1]), weights=a[0], density = True, histtype='step', color = 'b', fill=True, alpha = 0.5, label =  (r'$\sqrt{\langle x^2 \rangle} = $' + str(round(x_std,3)) + ' mm' ))
            ...
            if first_turn:
                ...
                if np.max(a[0]) > y_dat_lim:
                    y_dat_lim = round(np.max(a[0]), 2) * 1.2
    ...
    for i, file in enumerate(files):
        ...
        if not file_exists:     
            ...
            if first_turn:
                ...
                if (-1*y_dat_min) > (y_dat_max):
                    y_dat_lim = round(-1*y_dat_min, 5) * 1.5
                else:
                    y_dat_lim = round(y_dat_max, 5) * 1.5        
                print '\n\t ',x_lab,'-',y_lab,': ',x_lab,' limit set to ', x_dat_lim
                print '\n\t ',x_lab,'-',y_lab,': ',y_lab,' limit set to ', y_dat_lim
